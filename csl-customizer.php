...
function csl_frontpage_full_customizer($wp_customize) {
    ...
    /*--------------------------------------------------------------
    # Process Page - Case Studies Section
    --------------------------------------------------------------*/
    $wp_customize->add_section('process_case_studies_section', array(
        'title' => __('Process Page - Case Studies', 'auragrid'),
        'description' => __('Select which case studies to display on the Process page', 'auragrid'),
        'priority' => 160,
    ));

    // Get all published case studies for the dropdown
    $case_studies_options = array('' => '-- Select Case Study --');
    ...
    if ($case_studies_query->have_posts()) {
        while ($case_studies_query->have_posts()) {
            ...
            $case_studies_options[get_the_ID()] = get_the_title();
        }
        wp_reset_postdata();
    }

    // Case Study 1
    $wp_customize->add_setting('csl_process_case_study_1', array(
        'default' => '',
        'sanitize_callback' => 'absint',
    ));
    $wp_customize->add_control('csl_process_case_study_1', array(
        'type' => 'select',
        'section' => 'process_case_studies_section',
        'label' => __('Case Study 1', 'auragrid'),
        'choices' => $case_studies_options,
    ));

    // Case Study 2
    $wp_customize->add_setting('csl_process_case_study_2', array(
        'default' => '',
        'sanitize_callback' => 'absint',
    ));
    $wp_customize->add_control('csl_process_case_study_2', array(
        'type' => 'select',
        'section' => 'process_case_studies_section',
        'label' => __('Case Study 2', 'auragrid'),
        'choices' => $case_studies_options,
    ));

    // Case Study 3
    $wp_customize->add_setting('csl_process_case_study_3', array(
        'default' => '',
        'sanitize_callback' => 'absint',
    ));
    $wp_customize->add_control('csl_process_case_study_3', array(
        'type' => 'select',
        'section' => 'process_case_studies_section',
        'label' => __('Case Study 3', 'auragrid'),
        'choices' => $case_studies_options,
    ));
}
